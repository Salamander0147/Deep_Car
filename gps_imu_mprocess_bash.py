...
class Path:
    def __init__(self):
        dir_path = 'routes_data\SPKT_map_islab_khuA_oto_utm.csv'
...
class Car:
    ...
    def drive(self,lock):
        ...
        gps = get_gps(lock)
        # ACCELERATION
        accel= get_acceleration(lock)

        x_pre, y_pre, self.yaw = vehicle(gps,raw_yaw,self.pyaw)
        # print('original angle: ',self.yaw_ori)
        self.x, self.y = run_rlt(gps,raw_yaw,self.pyaw,accel,i) # Kalman coordinates
        i +=1
        # print('kalman angle: ',self.yaw) 
        # SAVE COORDINATE ORIGIN
        print('coordinates origin: ',x_pre, y_pre)
        x_ori.append(x_pre)
        y_ori.append(y_pre)
        df = pd.DataFrame({'x':x_ori, 'y':y_ori})
        df.to_csv('sensor_log/coor_origin_27_7_22_2.csv', index=False)
        # SAVE COORDINATE KALMAN
        print('coordinates Kalman: ',self.x, self.y)
        x_kal.append(self.x)
        y_kal.append(self.y)
        df = pd.DataFrame({'x':x_kal, 'y':y_kal})
        df.to_csv('sensor_log/coor_kalman_27_7_22_2.csv', index=False)
...
def get_acceleration(lock):
    '''
    Safely get acceleration from the temporary file.
    ...
    Returns
    -------
    acceleration : np.array of floats
        The acceleration of phone
    '''
    # Read the orientation data. Ensure the other thread has not opened the 
    # file at the same time
    global acceleration
    lock.acquire()    
    with open('sensor_log/accel.txt', 'rb') as file:
        file.seek(0)
        data = file.read().decode()
    lock.release()
    ...
    # Find and read in the orientation data
    entries = np.array(data.split(',')).astype(float)
    index = np.squeeze(np.where(entries == 3))
    acceleration = np.array(entries[index + 1 : index + 4])
    return acceleration
