...
class HandCodedLaneFollower(object):
    ...
    def follow_lane(self, frame, frame2):
        # Main entry point of the lane follower
        # show_image("road_mask", frame)
        # show_image("mid_mask", frame2)
...
def detect_lane(frame,frame2):
    ...
    middle_lanes= mid_lane(frame2)
    # if middle_lanes is not None:
    #     right_road,left_road=angles(frame,lane_lines,middle_lanes)
    #     # print('right co',right_road)
    #     # print('left co',left_road)
    #     right_road_angle=compute_steering_angle(frame,right_road)
    #     left_road_angle=compute_steering_angle(frame,left_road)
...
line_segments=[]
line_single=[]
def detect_line_segments_norm(cropped_edges):
    lines = cv2.HoughLines(cropped_edges,1,np.pi/180,100)
    if lines is not None:
        for i in range(0, len(lines)):
            ...
            x2 = int(x0 - 1000*(-b)) 
            y2 = int(y0 - 1000*(a))
        line_single=[x1,y1,x2,y2]
    line_segments.append(line_single)
    print('line',line_segments)
    return line_segments
...
middle_lane=[]
####################
def angles(frame,lane_lines,mid_lane):
